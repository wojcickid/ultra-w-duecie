...
- **Worker OAuth** (`worker/`, wdrażany razem ze stroną; konfiguracja w `wrangler.jsonc`; kod bez zależności czasu działania, własne lekkie typy; `wrangler` jest tylko w devDependencies):
  - `worker/index.ts` — punkt wejścia (`export default { fetch }`), uruchamiany dla KAŻDEGO żądania (`assets.run_worker_first: true`, DEC-014). Najpierw sprawdza protokół: `http:` → przekierowanie 301 na ten sam adres z `https:` (Custom Domain + statyczne pliki omijają zonowe Page/Redirect Rules Cloudflare, więc HTTPS trzeba wymusić w kodzie). Potem `GET /api/auth` i `GET /api/callback` trafiają do handlerów; inna metoda na tych ścieżkach → 405 (`Allow: GET`), inna ścieżka pod `/api/` → 404 (puste odpowiedzi, `no-store`); pozostały ruch (statyczne pliki) idzie do `env.ASSETS.fetch(request)`.
