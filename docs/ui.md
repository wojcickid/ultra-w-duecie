...
- **`RunCard`**: karta biegu (nazwa jako `h2`, znaczniki z `getRunBadges`, linijki wyników osób dla biegu nieukończonego wspólnie, dystans/miejsce/termin w `<dl>`); brakujące pola pomijane; cała karta klikalna. Termin z `getRunTerm` (jedno źródło prawdy dla osi czasu, `/biegi` i `/biegi/<id>`): gdy są wyniki z datą — najwcześniejsza data z wyników z etykietą „Ukończono” (wszyscy z datą ukończyli tego samego dnia), „Podejście” (ten sam dzień, ale np. DNF/DNS) albo „Pierwsze podejście” (wyniki z różnych dni, DEC-010); bez wyników — „Termin: <data planowana>” albo „Termin: orientacyjnie: <miesiąc>”.
