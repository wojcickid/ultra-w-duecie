...
- **`Timeline`**: oś czasu biegów jako `<ol>` w jednej kolumnie; znaczniki z `getRunBadges` (dla ukończonego wycofanego dwa), bieg wycofany z przerywaną ramką i adnotacją; gdy bieg nie jest ukończony wspólnie, pod spodem po jednej linijce na osobę (np. „Grzegorz: nie wystartował (DNS)”, z notatką po myślniku). Termin (etykieta dla czytników ekranu i data) pochodzi z `getRunTerm` (`src/lib/runs.ts`), tego samego helpera co `RunCard` i strona biegu.
- **`RunCard`**: karta biegu (nazwa jako `h2`, znaczniki z `getRunBadges`, linijki wyników osób dla biegu nieukończonego wspólnie, dystans/miejsce/termin w `<dl>`); brakujące pola pomijane; cała karta klikalna. Termin z `getRunTerm` (jedno źródło prawdy dla osi czasu, `/biegi` i `/biegi/<id>`): gdy są wyniki z datą — najwcześniejsza data z wyników z etykietą „Ukończono” (wszyscy z datą ukończyli tego samego dnia), „Podejście” (ten sam dzień, ale np. DNF/DNS) albo „Pierwsze podejście” (wyniki z różnych dni, DEC-010); bez wyników — jeden z trzech rodzajów terminu (`kind`, DEC-012): potwierdzona data („3 października 2026”, `plannedDate`), termin orientacyjny („październik 2027” z `expectedYear`, albo „październik” z dopiskiem „termin orientacyjny”; bez prefiksu „orientacyjnie:”) albo „Termin do ustalenia”. Widok pokazuje tekst z `value` i osobno dopisek z `qualifier` (jeśli jest); `formatRunTerm` skleja je w jeden tekst.
...
- **`PostCard`**: karta wpisu (tytuł `h2`, data, autorzy „Damian i Grzegorz” — zapis autorów wszędzie przez `joinNames` z `src/lib/blog.ts`: „Damian”, „Damian i Grzegorz”; opcjonalna miniatura z altem); cała karta klikalna.
